...
socket.addEventListener("message", ({ data }) => {
  ...
  if (message.id) {
    ...
    if (response.url.includes("/pages")) pageRequests.push({ url: response.url, status: response.status });
...
const fixedLayoutNative = await evaluate("Boolean(document.querySelector('foliate-view')) && !document.querySelector('.comic-stage')");
const mangaRTL = await evaluate("document.querySelector('foliate-view').book?.dir === 'rtl'");
...
await evaluate("(() => { const select = document.querySelector('.theme-settings select'); select.value = 'monet'; select.dispatchEvent(new Event('change', { bubbles: true })); document.querySelector('.theme-presets button')?.click(); })()");
await waitFor("document.documentElement.dataset.appTheme === 'monet'");
const systemTheme = await evaluate("document.querySelector('.user-panel').textContent.includes('莫奈调色盘') && document.querySelectorAll('.theme-presets button').length >= 4 && document.documentElement.dataset.appTheme === 'monet'");
await evaluate("document.querySelector('.user-panel > header button').click()");
await waitFor("document.querySelector('.account-popover')");
